...
### 9.1 Batch runner (`backend/run_benchmark.py`)

To compare experimental conditions across the three datasets with repeated runs
(per section 7), the batch runner drives the real domain services and produces
an isolated project + SQLite database per run, then evaluates each against the
...
- `full_llm` — the schema is generated by the LLM from the source documents,
  then populated by the LLM.
- `baseline` — the gold-standard schema is applied deterministically, then
  populated by the LLM (isolates population quality from schema generation).
...
### 9.2 Foreign-key integrity in generated databases

A regression was fixed in `backend/app/core/db_generator.py`: `ForeignKey`
instances must be passed positionally to `sqlalchemy.Column`. Previously they
were passed as the keyword argument `foreign_key=` (which SQLAlchemy does not
accept), so the constraint was silently dropped and no `FOREIGN KEY` clause was
emitted. The fix is covered by
`tests/test_population_pipeline.py::test_foreign_keys_are_built_into_the_generated_database`.
This matters for RQ2: FK violations are scored (cell-level FK errors), so the
generated databases must carry real referential constraints.

### 9.3 Column alignment for full-LLM runs

In the `full_llm` condition the LLM generates its own schema, whose names may
diverge from the gold schema while remaining semantically equivalent. Before
the cell-level comparison, `backend/app/evaluation/schema_alignment.py` builds
a table/column correspondence (frozen procedure per section 6.4):

1. exact normalised-name match (case-fold, snake-case, singular/plural);
2. substring fallback (both sides ≥ 3 characters to avoid false matches);
3. a per-dataset alias registry (`data/datasets/{name}/column_alias.json`,
...
Because the strict cell-level F1 scores identical information as wrong when the
synthetic keys differ, `backend/run_benchmark.py --adjudicate` adds a
**supplementary, qualitative** check (`backend/app/evaluation/
llm_adjudication.py`): the LLM-as-judge compares the generated schema and a
compact data dump against the gold schema and ground truth, scoring a fixed
three-dimension rubric (schema_equivalence, value_accuracy, completeness) on
0–100 at `temperature=0.0` with versioned rubric and prompt hashes.

This must be read alongside — never instead of — the deterministic F1: it
discriminates "genuinely wrong" from "represented differently" and surfaces
incompleteness that F1 alone may hide. It is subject to same-model
self-evaluation bias, so it is reported as a complementary signal with explicit
provenance and is not part of the primary RQ2 statistics.
