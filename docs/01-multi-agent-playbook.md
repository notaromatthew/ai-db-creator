...
| **Outputs** | `ParsedDocument` objects may contain parsed text and tabular structures, but persistence stores only a bounded `content_summary` (currently the first 5,000 characters). This is not a full-content archive. |
...
**Strategy A (bulk LLM SQL generation):** `generate_sql_for_population()` receives schema context and bounded parsed document summaries (at most 5,000 characters per document), not guaranteed complete file content. Its output and any deterministic fallback are provenance-labelled and require evaluation; neither is assumed correct.
