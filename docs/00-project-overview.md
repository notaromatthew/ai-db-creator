...
Once the schema is approved, the system sends the complete content of every uploaded document (CSV, Excel, PDF, TXT) to the LLM, which decides how to map values into the schema. The LLM is the primary route for every document type and overrides deterministic matching; it discards only duplicate rows already present in the target tables and accepts NULL/empty values where the schema allows them. The deterministic header matcher is retained strictly as a recovery path when the LLM returns no usable SQL:
