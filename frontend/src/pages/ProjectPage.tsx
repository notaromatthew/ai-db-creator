...
import { useExperiment } from '@/components/ExperimentShell'
...
export default function ProjectPage() {
  ...
  const latestSchemaEvent = [...interactions].reverse().find((event: any) =>
    ['schema_generated', 'schema_updated', 'schema_accepted'].includes(event.event_type),
  )

  const populateMut = useMutation({
    mutationFn: async (docIds: string[]) => { await emitRq4(id!,{type:'population_start',target_type:'project',target_name:'population',action:'start',phase:'population',operation_id:'population'}); return api.post(`/projects/${id}/populate`, { document_ids: docIds }) },
    onSuccess: (data: any) => {
      emitRq4(id!,{type:'population_complete',target_type:'project',target_name:'population',action:'complete',phase:'population',outcome:'success',operation_id:'population'}).catch(()=>{})
      const parts: string[] = []
    ...
    onError: (err: any) => {
      emitRq4(id!,{type:'validation_error',target_type:'project',target_name:'population',phase:'population',outcome:'failure',error_code:'POPULATION_ERROR',operation_id:'population'}).catch(()=>{})
    ...
    <div className="mx-auto max-w-6xl">
        ...
        <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
          <div>
            ...
            {latestSchemaEvent && <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
              Provenienza schema: {['schema_updated', 'schema_accepted'].includes(latestSchemaEvent.event_type) ? 'revisione umana' : `${latestSchemaEvent.data?.provider || 'IA'} · ${latestSchemaEvent.data?.model || 'modello non disponibile'}`} · run {String(latestSchemaEvent.run_id).slice(0, 8)} · confidenza non calibrata
            </p>}
          </div>
      ...
      {activeTab === 'schema' && (
        <div className="space-y-4">
          {can('chat') && <SchemaChat projectId={id!} schema={schema || null} documentIds={documents ? documents.map((d: any) => d.id) : []} />}

          {can('ai_generate') && <details className="surface p-4">
          ...
          {hasSchema && (
            <>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {can('import_sql') && <div className="relative inline-block">
                ...
                {can('populate') && <button onClick={handlePopulate} className="bg-green-600 text-white px-4 py-1 rounded text-sm disabled:opacity-50" disabled={populateMut.isPending}>
              ...
              {!showDiagram && (can('edit') ? <SchemaViewer schema={schema} projectId={id!} /> : <SchemaDiagram schema={schema} />)}
            </>
          )}
          {!hasSchema && (
      ...
      {activeTab === 'data' && hasSchema && can('edit') && <DataViewer projectId={id!} schema={schema} autoLoad />}
      {activeTab === 'query' && hasSchema && can('query') && <QueryBuilder projectId={id!} schema={schema} onDataChange={() => queryClient.invalidateQueries({ queryKey: ['stats', id] })} />}
